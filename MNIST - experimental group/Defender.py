...
def fl_trust(grad: torch.Tensor, validation_imgs: torch.Tensor, validation_label: torch.Tensor, model, batch_size,
    ...
    for index_i in top_ten_weight_index:
        ...
        current_AGR_param = torch.hstack((current_AGR_param, grad_zero[100 + index_i * 91: 100 + index_i * 91 + 91]))  # current_client_param = {Tensor:(91,)}
    ...
    print(grad_top_50_percent_param.shape) # torch.Size([50, 910])
    print(AGR_top_10_param_matrix.shape)    # torch.Size([910])

    scores = cos(grad_top_50_percent_param, AGR_top_10_param_matrix)
    scores = relu(scores)       # scores = {Tensor:(50,)}
    ...
    fl_trust_selected = torch.nn.functional.normalize(fl_trust_selected_still_be_50clients) * norm    # grad = {Tensor:(50,198660)}
