...
class ActivationsAndGradients:
    ...
    def __init__(self, model, target_layers, reshape_transform):
        ...
        #  target_layer.register_forward_hook() 是为每个层，注册一个 “正向传播” 的hook函数；当数据流过 这个layer时，就会给到save_activation()
    ...
    # 注意：正向和反向传播为Inverse order，所以正向用的是.append() 因为正向传播是由网络的底层流向高层
    ...
    # 注意：正向和反向传播为Inverse order，所以反向用的是插到最前面[0]的位置， 因为反向传播是由网络的高层流向底层
...
class FL_torch:
    ...
    def data_distribution(self, validation_size=300):
        ...
        self.validation_labels = self.test_labels[:validation_size]  # 这里上下 按照300为界，分别切出来test-set和 validation-set
    ...
    def eq_train(self):
        ...
        recorder = pd.DataFrame({"epoch": epoch_col, "attack_asr": test_acc_col, "test_loss": test_loss_col,
        ...
        recorder.to_csv(
            self.output_path + f"{self.dataset}_Ph_{self.Ph}_nH_{self.n_H}_MF_{self.malicious_factor}_K_{self.p_kernel}_def_{self.defender}"
                               f"_attack_{self.attack_mode}_start_{self.start_attack}_" + "_second_"+  str(end_count-start_count) +
            time_str +".csv")